 ...
 * @param act_code
 */
function validateRow(act_code, pageNum) {
    var total = 't_';
    ...
    var res = TextFieldArray.reduce(((acc, cur) => acc + parseCell(cur, function(name) {
                return document.getElementById(name.concat(pageNum));
        })), 0);
    document.getElementsByName(total + act_code)[pageNum].value = res;
}
 ...
 * @param col
 */
function calculateSum(col, pageNum)
{
    ...
    var res = TextFieldArray.reduce(((accumulator, current) => accumulator + parseCell(current, function(name) {
            return document.getElementsByName(name)[pageNum];
        })), 0);
    document.getElementsByName("sum")[pageNum].value = res;
}

function validateCol(col, pageNum) {
    ...
    document.getElementsByName("t_" + col)[pageNum].value = res;
}
/**
 ...
 * @param ref
 */
function checkValidTI(ref, pageNum) {
    var code = ref.name.substring(2,ref.name.length);
    var tag = ref.name.charAt(0);
    if(parseInt(code) < 21) {
        validateRow(code, pageNum); // validate && calculate row
        validateCol(tag, pageNum);   // validate && calculate col
    }
    calculateSum('t_', pageNum); // sum all fields with name beginning  with t_
}
